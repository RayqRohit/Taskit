    ...
    useEffect(() => {
        if (state.length) {
            const newModifiedState = state.filter((task) => {
                const condition = { filter: false, status: false, query: false };
                ...
                } else if (task.title.toLowerCase().includes(query.toLowerCase()))  {
                    condition.query = true;
                } 

                return condition.filter && condition.status && condition.query;
            });
